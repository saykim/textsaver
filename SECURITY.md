...
- **버전 2.4**: 구글 드라이브 백업 기능 추가 (최소 권한 `drive.file` 적용), 하이브리드 인증(Chrome/Edge 지원) 구현

## 🔐 구글 드라이브 연동 보안 (v2.4)

### 1. 최소 권한 원칙 (Least Privilege)
- **Scope**: `https://www.googleapis.com/auth/drive.file`
- **설명**: 확장 프로그램은 **자신이 생성한 파일**(`text_saver_backup.json`)에만 접근할 수 있습니다. 사용자의 기존 드라이브 파일(사진, 문서 등)에는 절대 접근할 수 없습니다.

### 2. 안전한 인증 흐름 (OAuth 2.0)
- **Chrome**: `chrome.identity.getAuthToken`을 사용하여 브라우저 내장 보안 인증을 수행합니다.
- **Edge/Others**: `chrome.identity.launchWebAuthFlow`를 사용하여 표준 OAuth2 웹 인증 흐름을 따릅니다.
- **토큰 관리**: 액세스 토큰은 브라우저의 메모리 내에서만 관리되며, 별도의 저장소에 영구 저장되지 않습니다. 